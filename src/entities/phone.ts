...
export class Phone {
  ...
  private static readonly VALID_DDD = [
  ...
  static random(maxAttempts = 100): Phone {
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      const ddd = Phone.VALID_DDD[Math.floor(Math.random() * Phone.VALID_DDD.length)] as string;
      const number = Math.floor(1000000 + Math.random() * 9000000);

      const data = [`${ddd}99${number}`, `${ddd}3${number}`][Math.floor(Math.random() * 2)] as string;
      const phone = new Phone(data);
      if (phone.isValid) {
        return phone;
      }
    }
    throw new Error('Falha ao gerar telefone aleatório após várias tentativas');
  }
}

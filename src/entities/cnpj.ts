...
export class CNPJ {
  ...
  get value(): string {
    return this._value.replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d+)/, '$1.$2.$3/$4-$5');
  }
  ...
  get isValid(): boolean {
    return [CNPJ.regex.test(this._value), CNPJ.check(this._value)].every(Boolean);
  }

  get masked(): string {
    return this._value.replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d+)/, '$1.***.***/$4-$5');
  }
